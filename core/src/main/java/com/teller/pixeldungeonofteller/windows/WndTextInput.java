...
public class WndTextInput extends Window {
    ...
    public WndTextInput(final String title, final String initialValue, final int maxLength,
        ...
        final int width;
        if (PixelDungeonOfTeller.landscape() && multiLine) {
            width = W_LAND_MULTI; //more editing space for landscape users
        } else {
            width = WIDTH;
        }

        PixelDungeonOfTeller.instance.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                ...
                textInput = new EditText(PixelDungeonOfTeller.instance);
                ...
                PixelDungeonOfTeller.instance.addContentView(textInput, layout);
            }
        });
    }
        ...
        if (textInput != null) {
            PixelDungeonOfTeller.instance.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    InputMethodManager imm = (InputMethodManager) PixelDungeonOfTeller
                    ...
                    PixelDungeonOfTeller.updateImmersiveMode();
