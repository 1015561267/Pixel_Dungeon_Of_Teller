    ...
    public WndStartGame(final int slot){
        ...
        if (Badges.isUnlocked(Badges.Badge.VICTORY)){
            IconButton challengeButton = new IconButton(
                    Icons.get( PixelDungeonOfTeller.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF)){
                @Override
                protected void onClick() {
                    PixelDungeonOfTeller.scene().add(new WndChallenges(PixelDungeonOfTeller.challenges(), true) {
                        public void onBackPressed() {
                            super.onBackPressed();
                            icon( Icons.get( PixelDungeonOfTeller.challenges() > 0 ?
            ...
            PixelDungeonOfTeller.challenges(0);
        }
